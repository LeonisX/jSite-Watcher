...
public class HtmlParser {
    ...
    public static String parse(byte[] html, String encoding) {
        ...
        htmlParser.setAsciiHtmlString(new String(html, 0, html.length > 1024 ? 1024 : html.length, StandardCharsets.ISO_8859_1).toLowerCase());
        if (encoding == null) {
            htmlParser.setPageEncoding(htmlParser.detectCharset());
        } else {
            htmlParser.setPageEncoding(new PageEncoding(encoding, Confidence.CERTAIN));
        }
        return "";
    }

    private PageEncoding detectCharset() {
        // TODO 4 prescan
        log.info("prescan");
        PageEncoding pageEncoding = doPrescan();
