...
public class Parser {
    ...
    public String parse() {
        ...
        //parser.setHtmlString(new String(html, 0, html.length > 1024 ? 1024 : html.length, StandardCharsets.ISO_8859_1).toLowerCase());
        try {
            //TODO move hack to separate class
            if (pageEncoding.getEncoding().isEmpty()) {
                pageEncoding.setEncoding("utf8");
            }
            htmlString = new String(html, pageEncoding.getEncoding());
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        htmlString = InputStreamPreprocessor.process(htmlString);
